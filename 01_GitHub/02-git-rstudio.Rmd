...
subtitle: Getting, Working On, and Submitting Your Assignments
output:
  pdf_document:
    toc: true
    toc_depth: '3'
...
You will be using git and GitHub to get your homework assignments, work on your homework, and submit your homework solutions. This tutorial will walk you through that process using git and GitHub through RStudio.

# Configuring GitHub Access Token for RStudio

Before you get started working on your homework, you will need to set up your Github access token for RStudio.

<div class="callout tip">
<span class="label">One-time setup</span>
You should only need to do this once. After your token is stored, RStudio will remember it for
future assignments.
</div>

First, install and load in these two packages in RStudio:

```{r install-packages, eval = FALSE}
# Install the two helper packages (only needed once per computer).
install.packages("usethis")
install.packages("gitcreds")

# Load them so their functions are available in this R session.
library(usethis)
library(gitcreds)
```
...
# Opens a GitHub page where you generate a Personal Access Token (PAT).
...
# Securely stores the token you just copied so RStudio can talk to GitHub.
gitcreds_set()
```
...
Each of you will be made members of the [`coredatascience-students` organization on GitHub](https://github.com/coredatascience-students). This means that your homework repositories all technically belong to us. But you will be granted unlimited access throughout the course!

You will notice when you visit the `coredatascience-students` [Github page](https://github.com/coredatascience-students) that you can only see repositories with your GitHub username on them. You will get one repository for each homework throughout the semester. When a new homework is released you can go to the corresponding repository to see what is in store. The work flow will be pretty simple.

1. Go to: https://github.com/coredatascience-students

2. Click on the repository you want to work on. For example `2025hw1-<your_GitHub_username>` for Homework 1.
...
1. If we do not see any more `commit`s two days after the deadline we will take the last `commit` as your final submission.
...
<span class="label">Watch the clock</span>
Every <code>commit</code> and <code>push</code> is time-stamped. Pushing after the deadline
counts as using a late day — so once you're finished, stop pushing.
</div>
